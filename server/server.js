...
//get by company
  app.get("/orderlist/:companyname", async (req, res) => {
    try {
  ...
  //get by sonumber
  app.get("orderlist/item/:sonumber",async(req,res) => {
    try {
...
//get by sonumber in companyname
  app.get("/orderlist/:companyname/:sonumber",async(req,res) => {
    try {
...
app.delete("/orderlist/:id", async (req, res) => {
    try {
      const { id } = req.params;
      const deleteitem = await pool.query("DELETE FROM orderlist WHERE aid = $1", [
        id
      ]);
